...
function createGame(color, playerToken, computerToken) {
    let newBoard = [
        [" ", " ", " ", " ", " ", " ", " "],
        [" ", " ", " ", " ", " ", " ", " "],
        [" ", " ", " ", " ", " ", " ", " "],
    ...
    gamesList[newGame.id] = newGame;
    return newGame;
}

function getGameFromID(gid) {
    return gamesList[gid];
}

function updateGameBoard(gid, move, isPlayer) {
    let game = gamesList[gid];
    ...
    let winner = checkWin(gid);
    if (winner !== " ") {
        gamesList[gid].finish = new Date().toDateString();
        if (winner === "X") {
            game.status = "VICTORY";
        } else if (winner === "O") {
            game.status = "LOSS";
    ...
    if (isPlayer) {
        ...
        while(!checkAvailable(compMove, gid)) {
            compMove = Math.floor(Math.random() * 7);
        }
        updateGameBoard(gid, compMove, false);
    }
    return game;
}

// Makes sure a move is valid
function checkAvailable(move, gid) {
    if (gamesList[gid].grid[0][move] != " ") {
        return false;
    }
    return true;
}
...
function checkWin(gid) {
    const gameBoard = gamesList[gid].grid;
    // Check rows
    for (let i = 0; i < 5; i++) {
        for (let j = 0; j < 4; j++) {
            if (
...
router.get("/", function (req, res, next) {
    res.send(Object.values(gamesList));
});
...
router.get("/gids/:gid", function (req, res, next) {
    ...
    res.status(200).send(getGameFromID(gid));
});

router.post("/gids/:gid", function (req, res, next) {
    const { gid } = req.params;
    res.status(200).send(updateGameBoard(gid, req.query.move, true));
});
